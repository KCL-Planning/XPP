...
def run():
    ...
    cmd = "dot -Tpdf " + resultFolder + "/graph.gv -o  " + resultFolder + "/graph.pdf"
    os.system(cmd)

def generateProperties(problemPath):
    problem_name = getFileName(problemPath)

    parent_path = problemPath.replace("/" + problem_name,"")
    problem_folder = parent_path + "/" + problem_name.replace(".pddl","")

    os.makedirs(problem_folder)
    os.makedirs(problem_folder + "/" + "properties")
    os.makedirs(problem_folder + "/" + "task")
    os.makedirs(problem_folder + "/" + "results")

    cmd = "/./" + solver + "  --build release64 " + problemPath + " --translate-options --property_compilation_type  1 --properties_folder " + problem_folder + "/" + "properties" + " --search-options --heuristic \"h=hmax()\" --search \"astar(hmax())\""
    os.system(cmd)


    cmd = "cp " + problemPath + " " + problem_folder + "/task/problem.pddl"
    os.system(cmd)
    cmd = "cp " + parent_path + "/domain.pddl " + problem_folder + "/task/"
    os.system(cmd)
...
if mode == "-gen":
    ...
    solver = sys.argv[2]
    problemPath = sys.argv[3]

    problemRootFolder = generateProperties(problemPath)

    problemFolder = problemRootFolder + "/task"
    propertyFolder = problemRootFolder + "/properties"
    resultFolder = problemRootFolder + "/results"
    graph_file = resultFolder + "/graph.gv"  
    run()
