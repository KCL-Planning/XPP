...
\modelsof{\plans}{\propq}$. The subset of \props\ \plans-equivalent
with \prop\ is written $\equiv{\plans}{\prop} := \{\propq \mid \propq
\in \props, \iff{\plans}{\prop}{\propq}\}$.
\end{definition}
...
compact and/or (ii) more restricted plan-space explanations. Towards
(i), a technique we will use in our concrete instantiation of this
framework is \defined{subsumption} over \plans-entailment relations,
relying on a sufficient criterion for \plans-entailment:
...
The role of \entailsuff\ in Definition~\ref{def:dcpdo} is to qualify
...
$\equiv{\plans}{\prop})$) instead of a dominant cPDO (at the level of
plan properties \prop). This alternative incurs complications as
...
which is not necessarily the case for a ``cognitively easy''
sufficient criterion. Nevertheless, dominant PDOs may be preferrable
to dominant cPDOs in some application scenarios.

As a form of (ii) more restricted plan-space explanations, what we
will use in our concrete instantiation of this framework is simply the
exclusive focus on a predefined subset \deps\ of dependencies of
interest:
...
plan properties. Then a (dc)PDO for $\deps$ results from ignoring
\plans-entailments $\entails{\plans}{\prop}{\propq}$ where $(\prop,\propq)
\not\in \deps$.
\end{definition}
