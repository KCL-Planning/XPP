...
experiment where we scaled question size $|A|$ from $1$ to $5$, and
randomly selected $10$ questions of each
size. Figure~\ref{fig:ipc-local} shows the data. As expected, both
computational effort and explanation size decrease with $|A|$, \ie,
with how specific the user question is.

\joerg{add a per-domain plot in the fig?}
...
\end{tabular}
\vspace{-0.3cm}
\caption{\label{fig:ipc-local} Left: coverage (blue, right y-axis),
average runtime (red, left y-axis). Right: average \#MUGS. Both x-axis
$|A|$.\joerg{optimize format: y-axis labels, b/w compatible instead of
red and blue, curve identifiers in left plot}}
\vspace{-0.4cm}
\end{figure}
