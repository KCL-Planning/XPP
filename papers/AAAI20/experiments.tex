...
\joerg{presumably: 1 plot of coverage (in percent of total), including all 
instances (and thus domains) with at least 5 goals; 1 plot of
average \#MUGS, including all instances (and thus domains) where for
every question size at least one question was answered}


\begin{figure}[ht]
\input{tables/all_points.tex}
	\caption{ Search time for question size 1 to 5 with at most 10
		questions per size.  Average over all problems where
		at least one run per question size is solved.  }
\end{figure}
