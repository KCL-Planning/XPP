...
\begin{figure*}[h!]
	...
	\centering \input{tables/coverage_IPC_max_mugs.tex} 
        \vspace{-0.2cm}
	\caption{Results on IPC benchmarks modified for oversubscription planning. Reference Points: related classical planning tasks (see text). Coverage: of our MUGS algorithms SysS and SysW, with vs.\ without conjunction learning \hc. \#MUGS: average/maximum number of MUGS, indicating explanation size (see text). Search Tree Fraction: fraction of worst-case search tree explored. Best performance in each part shown in \textbf{boldface}. Cost bounds set to $x$ times optimal cost. \joerg{move \#MUGS to left?} \joerg{add comparison to OSP planner? is more direct, and could be quite a bit more favorable for us ...}}
...
To evaluate the computation of local explanations -- the entailments
of a soft-goal conjunction $\bigwedge_{a \in A} a$ -- we set up an
experiment where we used only benchmark instances with $\geq 6$ goals,
scaled question size $|A|$ from $1$ to $5$, and randomly selected $10$
questions of each size. Figure~\ref{fig:ipc-local} shows the data. As
expected, both computational effort and explanation size decrease with
$|A|$, \ie, with how specific the user question is. 
...
\end{tabular}
\vspace{-0.3cm}
\caption{\label{fig:ipc-local} Left: coverage (blue, right y-axis) and
average runtime (red, left y-axis) for SysS, as a function of
$|A|$. Right: average \#MUGS as a function of $|A|$. Averages taken
over instances where all questions were solved.\joerg{optimize format:
y-axis labels, b/w compatible instead of red and blue, curve
identifiers in left plot}}
\vspace{-0.6cm}
\end{figure}
